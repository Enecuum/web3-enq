...
let utils = {
	...
	hash_pos : function(pos){
		if (!pos)
			return undefined;
		let str = ['id','owner','fee','name'].map(v => crypto.createHash('sha256').update(pos[v].toString().toLowerCase()).digest('hex')).join("");
		return crypto.createHash('sha256').update(str).digest('hex');
	},
	...
	valid_leader_sign(mblocks, LPoSID, cfg_ecc){
	...
	valid_full_microblocks(mblocks, accounts, tokens, check_txs_sign){
		...
		mblocks = mblocks.filter((mblock)=>{
			...
			if(tok_idx < 0){
				console.warn(`ignoring mblock ${JSON.stringify(mblock)} : token not found`);
				return false;
			}
			if(tokens[tok_idx].minable !== 1){
				console.warn(`ignoring mblock ${JSON.stringify(mblock)} : token not minable`);
				return false;
			}
			let pub = accounts.findIndex(a => ((a.id === mblock.publisher) && ((a.token === mblock.token))));
			if (pub < 0){
				console.warn(`ignoring mblock ${JSON.stringify(mblock)} : publisher not found`);
				return false;
			}
			if ((accounts[pub].amount < tokens[tok_idx].min_stake)){
				console.warn(`ignoring mblock ${JSON.stringify(mblock)} due to low stake`);
				return false;
			}

		 	mblock.hash = this.hash_mblock(mblock);
		 	let signed_msg = mblock.hash + (mblock.referrer ? (mblock.referrer) : "") + mblock.token;
		...
		return sblocks.filter(s => {
			...
			if (pub > -1) {
				if(!top_poses.some(a => pos_stakes[pub].pos_id === a.pos_id)){
					console.warn(`ignoring sblock ${JSON.stringify(s)} contract is not in top poses`);
					return false;
				}
				if (pos_stakes[pub].self_del >= pos_min_stake) {
					return true;
				} else {
					console.warn(`ignoring sblock ${JSON.stringify(s)} due to low stake`);
					return false;
				}
			} else {
				console.warn(`ignoring sblock ${JSON.stringify(s)} contract not found`);
				return false;
			}
		});
	},
    exist_native_token(mblocks){
        return mblocks.some(a => a.token === this.ENQ_TOKEN_NAME);
    },
	 ...
	 * @return {number}
	 */
	PID : function(input, target_speed){
	   let now = Date.now();
	   let timeChange = (now - this.lastTime)/1000;
//	   if(timeChange >= this.PID_TIMEOUT)
//	   {
	      ...
	      let dInput = (error - this.lastError) / timeChange;
	      
	      /*Compute PID Output*/
	      let output = this.kp * error +  this.ITerm + this.kd * dInput;
	      ...
	      this.pid_cached = output;
		  return output;
//	   }
//	   else {
//	   	return this.pid_cached;
//	   }
	},
	...
	calc_difficulty : async function(db, target_speed, kblock) {
		let time = await db.get_time_delta_kblock(kblock.link, this.MINER_CHECK_TARGET_INTERVAL);
		if (time === undefined) {
			console.trace("undefined time delta kblock");
			return 0;
		}
		let delta_time_1 = Number(time['time']) / Number(this.MINER_CHECK_TARGET_INTERVAL);
		let data = await db.get_avg_diff_kblock(kblock.link, this.MINER_CHECK_TARGET_INTERVAL);
		let avg_diff_1 = Number(data['avg_diff']);
		let diff_offset = this.PID(delta_time_1, target_speed);
